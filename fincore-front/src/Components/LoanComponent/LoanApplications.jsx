            ...
            <div style={loanStyles.tableWrap}>
              <table style={loanStyles.table}>
                ...
                <tbody>
                 { 
                  applications.map((a) => (
                    <tr key={a.customerLoanId}>{console.log(a)}
                      <td style={loanStyles.td}>  
                        <strong>{a.customerLoanId}</strong>
                      </td>
                          ...
                          {a.status === "APPROVED" &&
                            a.outstandingAmount > 0 && (
                              <button
                                style={loanStyles.primaryButton}
                                onClick={() =>
                                  navigate(`/loan-repayment/${a.customerLoanId}`)
                                }
                              >
                                Make Repayment
                              </button>
          ...
          <div style={loanStyles.detailView.detailsModal}>
            <div style={loanStyles.detailView.modalHeader}>
              <div>
                ...
                <h2 style={loanStyles.detailView.modalTitle}>{selected.customerLoanId}</h2>
              </div>
              <button
            ...
            <div style={loanStyles.detailView.detailsBox}>
              {detailRow("Application ID", selected.customerLoanId)}
              ...
              {detailRow(
                ...
                selected.totalTenure
                  ? `${selected.totalTenure} ${Number(selected.totalTenure) === 1 ? "year" : "years"}`
                  : "—",
              )}
              ...
              {detailRow("Application Date", selected.loanDate || "—")}
              {detailRow("Review Date", selected.reviewedDate || "Not reviewed yet")}
              {detailRow("Application Status", selected.status)}
              ...
              {selected.status === "APPROVED" &&
                selected.outstandingAmount > 0 && (
                  <button
                    ...
                    onClick={() => {
                      setSelected(null);
                      navigate(`/loan-repayment/${selected.customerLoanId}`);
                    }}
                  >
                    Make Repayment
                  </button>
